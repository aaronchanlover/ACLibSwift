...
/// 对原生的UITabBarItemPositioning进行扩展，通过UITabBarItemPositioning设置时，系统会自动添加insets，这使得添加背景样式的需求变得不可能实现。ACTabBarItemPositioning完全支持原有的item Position 类型，除此之外还支持完全fill模式。
///
...
public enum ACTabBarItemPositioning : Int {
...
internal protocol ACTabBarDelegate: NSObjectProtocol {
...
/// ACTabBar是高度自定义的UITabBar子类，通过添加UIControl的方式实现自定义tabBarItem的效果。目前支持tabBar的大部分属性的设置，例如delegate,items,selectedImge,itemPositioning,itemWidth,itemSpacing等，以后会更加细致的优化tabBar原有属性的设置效果。
open class ACTabBar: UITabBar {
    
    internal weak var customDelegate: ACTabBarDelegate?
    ...
    public var itemCustomPositioning: ACTabBarItemPositioning? {
        didSet {
    ...
    internal var containers = [ACTabBarItemContainer]()
    /// 缓存当前tabBarController用来判断是否存在"More"Tab
    internal weak var tabBarController: UITabBarController?
    /// 自定义'More'按钮样式，继承自ACTabBarItemContentView
    open var moreContentView: ACTabBarItemContentView? = ACTabBarItemMoreContentView.init() {
        didSet { self.reload() }
    }
    ...
    open override func beginCustomizingItems(_ items: [UITabBarItem]) {
        ACTabBarController.printError("beginCustomizingItems(_:) is unsupported in ACTabBar.")
        super.beginCustomizingItems(items)
    }
    
    open override func endCustomizing(animated: Bool) -> Bool {
        ACTabBarController.printError("endCustomizing(_:) is unsupported in ACTabBar.")
...
internal extension ACTabBar /* Layout */ {
    
    internal func updateLayout() {
        guard let tabBarItems = self.items else {
            ACTabBarController.printError("empty items")
            return
        }
...
internal extension ACTabBar /* Actions */ {
    
    internal func isMoreItem(_ index: Int) -> Bool {
        return ACTabBarController.isShowingMore(tabBarController) && (index == (items?.count ?? 0) - 1)
    }
    ...
    internal func reload() {
        ...
        for (idx, item) in tabBarItems.enumerated() {
            let container = ACTabBarItemContainer.init(self, tag: 1000 + idx)
            self.addSubview(container)
            self.containers.append(container)
            
            if let item = item as? ACTabBarItem, let contentView = item.contentView {
    ...
    internal func highlightAction(_ sender: AnyObject?) {
        ...
        if let item = item as? ACTabBarItem {
    ...
    internal func select(itemAtIndex idx: Int, animated: Bool) {
        ...
        if currentIndex != newIndex {
            if currentIndex != -1 && currentIndex < items?.count ?? 0{
                if let currentItem = items?[currentIndex] as? ACTabBarItem {
